...
Three new options let you override the default colors of the `clock` clear-state display without writing CSS. ([#124](https://github.com/djdevil/AlertTicker-Card/issues/124))
...
clear_clock_background: "#0a0a1a"
```

| Option | Description |
|--------|-------------|
| `clear_clock_color` | CSS color for the clock digits |
| `clear_clock_date_color` | CSS color for the date text |
| `clear_clock_background` | CSS color/gradient for the card background |

All three are optional and compatible with all clock styles (`aurora`, `gold`, `matrix`). Color pickers available in the visual editor under **✅ All Clear → Custom colors**.

---

### 🌡️ Custom Sensor Overrides for the Weather Widget

Five new optional config keys let you replace individual weather fields with readings from your own local sensors — no template `weather.*` entity required. ([#123](https://github.com/djdevil/AlertTicker-Card/issues/123))
...
| `clear_weather_aqi_entity` | Add air quality / PM2.5 as a new badge row (🌿) |
...
| # | Fix |
|---|-----|
| [#119](https://github.com/djdevil/AlertTicker-Card/issues/119) | **Music player cover art disappears in HA light themes** — missing `position: relative` on the player container caused the art background layer to resolve a wrong containing block in light themes; fixed + added explicit dark base color |
| [#113](https://github.com/djdevil/AlertTicker-Card/issues/113) | **Manual navigation always resets to first alert** — `_preview_index` was re-applied on every HA state update; fixed to only jump when the editor actually changes the preview selection |
| — | **`{%`-only Jinja2 templates not rendered in overlay** — control-flow-only templates bypassed the WebSocket render path; fixed |
| — | **Camera stream audio persists after overlay dismissal** — stream element now removed from DOM on hide, stopping all media tracks |
| — | **Camera snapshot dimensions inconsistent in overlay** — same wrapper fix as live stream for correct aspect ratio and border-radius |
| — | **`clear_clock_background` ignored on aurora/gold/matrix styles** — hardcoded gradients were overriding the CSS variable; fixed with `var()` fallback pattern |
| — | **Weather high/low missing in plain `weather` mode** — forecast subscription was only started for `weather_forecast`/`forecast` modes; now also started for plain `weather` |
| — | **History shows raw Jinja2 for complex templates** — 500 ms async cancel timer replaced with a `done`-flag pattern and 5 s abort |
| — | **History recorded during visual editor preview** — `_isEditMode()` guard added to prevent phantom history entries |
| — | **`state: "{{ }}"` trigger templates intermittently cleared** — trigger templates now included in the WS subscription keep-alive set |
| — | **Test mode preview broken with YAML-pasted configs** — alerts with non-existent entities are no longer filtered out in test mode, enabling full YAML preview |
| — | **Humidity hidden in `stage` weather theme** — humidity moved to the correct badge row visible in all themes |
